...
function coverUrl(item) {
  ...
  return `./assets/images/amiibo/${cover}.png`;
}

function getFilteredItems() {
  return state.items.filter((item) => (state.showOwned ? isOwned(item) : !isOwned(item)));
}
...
function renderGrid() {
  ...
  setTimeout(() => {
    ...
    if (!items.length) {
      container.appendChild(el("p", { class: "empty-state" }, "No amiibos found."));
    } else {
      items.forEach((item) => {
