...
function renderGrid() {
  ...
  setTimeout(() => {
    ...
    if (!items.length) {
      container.appendChild(el("p", { class: "empty-state" }, "No games found."));
    } else {
      items.forEach((item) => {
...
function buildHeader() {
  ...
  const digitalBtn = el("button", { id: "toggle-format" }, "Digital");
  digitalBtn.addEventListener("click", () => {
    state.format = state.format === "physical" ? "digital" : "physical";
    digitalBtn.textContent = state.format === "physical" ? "Digital" : "Physical";
    renderGrid();
  });
  header.appendChild(digitalBtn);
